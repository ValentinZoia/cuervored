...
export interface UseInfinitePostsProps {
  ...
  isUserPagePosts?: boolean; //esto significa que es una pagina de un usuario, no son los post del feed, sino que son los post de un useuario en especial, por lo tanto la queryKey vendria a ser el userId.
}

export function useInfinitePosts({
  queryKey,
  fetchFn,
  isUserPagePosts = false,
  ...
  return useSuspenseInfiniteQuery({
    ...
    queryFn: ({ pageParam }) =>
      isUserPagePosts ? fetchFn({ pageParam, userId }) : fetchFn({ pageParam }),
